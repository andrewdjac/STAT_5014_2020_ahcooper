...
The minimum number of iterations was `r min_iters` when our starting value for $\theta_0$ was `r grid_inds[which.min(iterations),][1]` and our starting values for $\theta_1$ was `r grid_inds[which.min(iterations),][2]`.

The maximum number of iterations was `r max_iters` when our starting value for $\theta_0$ was `r grid_inds[which.max(iterations),][1]` and our starting values for $\theta_1$ was `r grid_inds[which.max(iterations),][2]`.
...
q <- sample(c(0, 0.5, 1), size = 668, replace = T)
A <- C[id, -id]
B <- C[-id, -id]
p <- runif(932, 0, 1)
r <- runif(668, 0, 1)
